...
````
zval里的refcout\_\_gc是zend\_uint类型，也就是unsinged int型，is\_ref\_\_gc和type则是unsigned char型的。<br />保存变量值的value则是zvalue\_value类型(PHP5)，它是一个Union，同样定义在了Zend/zend.h文件里：
````c
